...
std::string program_line::eliminatespace(std::string cmd){
    int i=0;int j=0;
    char* cmdd;
    std::string desti;
...
int program_line::runline(QVector<expression>& variables,int &printnumber,statement & statement){
    int i=0;
...
//                int i_1=0;
//                for(i_1=0;i_1<next.length();i_1++){
//                    if(next[i_1]!=' '&&next[i_1+1]==' '){
//                        break;
//                    }
//                }
//                if(i_1==next.length()){return -1;}
...
//                }
            ...
            if(type.compare("PRINT")==0){
                ...
                if(i_1==int(commands.length())){
                    ...
                    int a;
                    expressions tmp;
                    tmp.addexpression(next,variables,a,false,false);
                    printnumber=a;
                    return -2;
                    }
           ...
           if(type.compare("IF")==0){
                ...
                if(flag==true){
                    ...
                    if(i_5==int(commands.size())) return -1;
                    int i_6=i_5;
                    ...
                    if(i_6==int(commands.size())) return -1;
                    std::string pd(commands.begin()+i_5+1,commands.begin()+i_6+1);
                    if(pd.compare("THEN")!=0) return -1;
                    else{
                        int i_7=0;
                    ...
                    statement.relativecounttimes+=1;
                    return -9999;
                }
           }
    }
    else{
        if(commands.compare("END")==0){
            return -1;
        }
    }
}

bool program_line::comparing(int a, char b, int c){
    if(b=='>'){
        if(a>c) return true;
        else return false;
    }
